node() {
    ...
    stage('Import results to Xray') {
        ...
        def info = '''{
            ...
            step([$class: 'XrayImportBuilder', projectKey: Xray-Test, endpointName: '/junit', importFilePath: 'reports/*.xml', importInfo: info, inputInfoSwitcher: 'fileContent', serverInstance: xrayConnectorId])
        }
    stage('cleanWs') {
        echo "\n\nCleanWs"
        cleanWs()
